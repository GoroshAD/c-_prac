...
int main(int argc, char *argv[]) {
    ...
    while (no_improvement_counter < 10) {
        ...
        for (int i = 0; i < processors; ++i) {
            mq_receive(to, buffer, 8192, &prio);
            //std::cout << "serv received 1" << std::endl;
            Schedule_solution *sol = receiving_parser(buffer, n, m, jobs);
        ...
        for (int i = 0; i < processors; ++i) {
            if (no_improvement_counter == 10) {
                mq_send(from, sending_parser(best_sol, n, 2, buffer), 8192, prio);
                //std::cout << "serv send 2" << std::endl;
            } else {
                mq_send(from, sending_parser(best_sol, n, 1, buffer), 8192, prio);
